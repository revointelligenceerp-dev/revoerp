...
import { useNotaFiscal } from '../hooks/data/useNotaFiscal';
import { StatusFatura } from '../types';

export const NotasFiscais: React.FC = () => {
  const { faturas, loading, error, currentPage, totalPages, goToPage } = useNotaFiscal();
    ...
    if (loading && faturas.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center text-center py-10 space-y-4">
          ...
          <p className="text-gray-600 font-medium">Carregando notas fiscais...</p>
        </div>
      );
    }
    ...
    if (faturasFiltradas.length === 0) {
      return (
        <div className="text-center py-10 text-gray-500">
          ...
          <p className="text-sm">Fature um pedido de venda para que a nota apareça aqui.</p>
        </div>
      );
    }
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/20">
              <th className="text-left py-4 px-2 font-medium text-gray-700">Nota Fiscal</th>
  ...
  return (
    <div>
      <Header 
        title="Notas Fiscais" 
        subtitle="Gerencie suas notas fiscais emitidas"
      />
