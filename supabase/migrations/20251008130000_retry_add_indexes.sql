-- Habilita a extensão para busca textual mais rápida, se não existir
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- Adiciona a coluna 'codigo' à tabela 'servicos' se ela não existir
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name='servicos' AND column_name='codigo'
  ) THEN
...
-- Adiciona a coluna 'situacao' à tabela 'servicos' se ela não existir
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name='servicos' AND column_name='situacao'
  ) THEN
...
-- Cria índices para otimizar buscas e filtros
-- Índices em chaves estrangeiras são cruciais para performance de JOINs
...
CREATE INDEX IF NOT EXISTS idx_ordem_servico_itens_os_id ON public.ordem_servico_itens (ordem_servico_id);
CREATE INDEX IF NOT EXISTS idx_ordem_servico_itens_servico_id ON public.ordem_servico_itens (servico_id);
