...
DO $$
BEGIN
    -- Apaga tipos antigos, se existirem, para garantir a recriação correta
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'contribuinte_icms') THEN
        BEGIN
            ...
            DROP TYPE public.contribuinte_icms;
        EXCEPTION WHEN dependent_objects_still_exist THEN
            -- Ignora se o tipo estiver em uso, pois será recriado
        END;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'origem_produto') THEN
        BEGIN
            ALTER TABLE public.produtos ALTER COLUMN origem TYPE text;
            DROP TYPE public.origem_produto;
        EXCEPTION WHEN dependent_objects_still_exist THEN
        END;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_pessoa_vendedor') THEN
        BEGIN
            ALTER TABLE public.vendedores ALTER COLUMN tipo_pessoa TYPE text;
            DROP TYPE public.tipo_pessoa_vendedor;
        EXCEPTION WHEN dependent_objects_still_exist THEN
        END;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'situacao_vendedor') THEN
        BEGIN
            ALTER TABLE public.vendedores ALTER COLUMN situacao TYPE text;
            DROP TYPE public.situacao_vendedor;
        EXCEPTION WHEN dependent_objects_still_exist THEN
        END;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'regra_comissao') THEN
        BEGIN
            ALTER TABLE public.vendedores ALTER COLUMN regra_liberacao_comissao TYPE text;
            DROP TYPE public.regra_comissao;
        EXCEPTION WHEN dependent_objects_still_exist THEN
        END;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_comissao_vendedor') THEN
        BEGIN
            ALTER TABLE public.vendedores ALTER COLUMN tipo_comissao TYPE text;
            DROP TYPE public.tipo_comissao_vendedor;
        EXCEPTION WHEN dependent_objects_still_exist THEN
        END;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'frete_por_conta') THEN
        BEGIN
            ALTER TABLE public.ordens_compra ALTER COLUMN frete_por_conta TYPE text;
            DROP TYPE public.frete_por_conta;
        EXCEPTION WHEN dependent_objects_still_exist THEN
        END;
    END IF;

    -- Recria os ENUMs com chaves curtas
    CREATE TYPE public.contribuinte_icms AS ENUM ('NAO_INFORMADO', 'CONTRIBUINTE_ICMS', 'CONTRIBUINTE_ISENTO', 'NAO_CONTRIBUINTE');
    CREATE TYPE public.origem_produto AS ENUM ('NACIONAL_0', 'ESTRANGEIRA_DIRETA_1', 'ESTRANGEIRA_INTERNO_2', 'NACIONAL_CONTEUDO_40_70_3', 'NACIONAL_PROCESSO_BASICO_4', 'NACIONAL_CONTEUDO_INF_40_5', 'ESTRANGEIRA_DIRETA_SEM_SIMILAR_6', 'ESTRANGEIRA_INTERNO_SEM_SIMILAR_7', 'NACIONAL_CONTEUDO_SUP_70_8');
    ...
    CREATE TYPE public.tipo_comissao_vendedor AS ENUM ('FIXA', 'POR_DESCONTO');
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'ocorrencia_conta') THEN CREATE TYPE public.ocorrencia_conta AS ENUM ('Única', 'Recorrente'); END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_embalagem_produto') THEN CREATE TYPE public.tipo_embalagem_produto AS ENUM ('Pacote / Caixa', 'Envelope', 'Rolo / Cilíndrico'); END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'embalagem_produto') THEN CREATE TYPE public.embalagem_produto AS ENUM ('Embalagem customizada', 'Caixa de Encomenda Flex', 'Caixa de Encomenda CE – 01', 'Caixa de Encomenda CE – 02', 'Caixa de Encomenda CE – 03', 'Caixa de Encomenda CE – 07', 'Caixa de Encomenda 5B', 'Caixa de Encomenda 6B', 'Caixa de Encomenda Vai e Vem', 'Caixa de Encomenda B', 'Caixa de Encomenda 2B', 'Caixa de Encomenda 4B', 'Caixa de Encomenda Temática 01', 'Caixa de Encomenda Temática 02', 'Caixa de Encomenda Temática 03', 'Criar nova embalagem ...'); END IF;
...
CREATE TABLE IF NOT EXISTS public.pedido_venda_anexos (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, pedido_id uuid NOT NULL REFERENCES public.pedidos_venda(id) ON DELETE CASCADE, nome_arquivo text NOT NULL, path text NOT NULL, tamanho integer NOT NULL, tipo text NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.faturas_venda (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, pedido_id uuid NOT NULL REFERENCES public.pedidos_venda(id), numero_fatura text NOT NULL, data_emissao date NOT NULL, data_vencimento date NOT NULL, valor_total numeric NOT NULL, status public.status_fatura NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.fatura_venda_itens (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, fatura_id uuid NOT NULL REFERENCES public.faturas_venda(id) ON DELETE CASCADE, descricao text NOT NULL, quantidade numeric NOT NULL, valor_unitario numeric NOT NULL, valor_total numeric NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
...
CREATE TABLE IF NOT EXISTS public.contas_receber_anexos (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, conta_receber_id uuid NOT NULL REFERENCES public.contas_receber(id) ON DELETE CASCADE, nome_arquivo text NOT NULL, path text NOT NULL, tamanho integer NOT NULL, tipo text NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.contas_pagar (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, descricao text NOT NULL, valor numeric NOT NULL, data_vencimento date NOT NULL, data_pagamento date, status public.status_conta_pagar NOT NULL, fornecedor_id uuid REFERENCES public.clientes(id), forma_pagamento text, numero_documento text, historico text, categoria_id text, ocorrencia public.ocorrencia_conta NOT NULL, competencia text, marcadores text[], created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.contas_pagar_anexos (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, conta_pagar_id uuid NOT NULL REFERENCES public.contas_pagar(id) ON DELETE CASCADE, nome_arquivo text NOT NULL, path text NOT NULL, tamanho integer NOT NULL, tipo text NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.fluxo_caixa (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, data date NOT NULL, descricao text NOT NULL, valor numeric NOT NULL, tipo public.tipo_movimento_caixa NOT NULL, conta_receber_id uuid REFERENCES public.contas_receber(id), conta_pagar_id uuid REFERENCES public.contas_pagar(id), created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.ordens_servico (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, numero text NOT NULL, cliente_id uuid NOT NULL REFERENCES public.clientes(id), descricao_servico text, consideracoes_finais text, data_inicio date NOT NULL, data_previsao date, hora time, data_conclusao date, total_servicos numeric NOT NULL, desconto text, observacoes_servico text, observacoes_internas text, vendedor_id uuid REFERENCES public.vendedores(id), comissao_percentual numeric, valor_comissao numeric, tecnico_id uuid REFERENCES public.vendedores(id), orcar boolean DEFAULT false, forma_recebimento text, meio_pagamento text, conta_bancaria text, categoria_financeira text, condicao_pagamento text, marcadores text[], status public.status_os NOT NULL, prioridade public.prioridade_os NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.ordem_servico_itens (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, ordem_servico_id uuid NOT NULL REFERENCES public.ordens_servico(id) ON DELETE CASCADE, servico_id uuid REFERENCES public.servicos(id), descricao text NOT NULL, codigo text, quantidade numeric NOT NULL, preco numeric NOT NULL, desconto numeric, valor_total numeric NOT NULL, orcar boolean, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.ordem_servico_anexos (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, ordem_servico_id uuid NOT NULL REFERENCES public.ordens_servico(id) ON DELETE CASCADE, nome_arquivo text NOT NULL, path text NOT NULL, tamanho integer NOT NULL, tipo text NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.ordens_compra (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, fornecedor_id uuid NOT NULL REFERENCES public.clientes(id), numero text NOT NULL, data_compra date NOT NULL, data_prevista date, total_produtos numeric, desconto text, frete numeric, total_ipi numeric, total_icms_st numeric, total_geral numeric NOT NULL, numero_no_fornecedor text, condicao_pagamento text, categoria_id text, transportador_nome text, frete_por_conta public.frete_por_conta, observacoes text, marcadores text[], observacoes_internas text, status public.status_ordem_compra, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.ordem_compra_itens (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, ordem_compra_id uuid NOT NULL REFERENCES public.ordens_compra(id) ON DELETE CASCADE, produto_id uuid REFERENCES public.produtos(id), descricao text NOT NULL, codigo text, gtin_ean text, quantidade numeric NOT NULL, unidade text, preco_unitario numeric NOT NULL, ipi numeric, preco_total numeric NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.ordem_compra_anexos (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, ordem_compra_id uuid NOT NULL REFERENCES public.ordens_compra(id) ON DELETE CASCADE, nome_arquivo text NOT NULL, path text NOT NULL, tamanho integer NOT NULL, tipo text NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.expedicoes (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, lote text NOT NULL, forma_envio text NOT NULL, status public.status_expedicao NOT NULL, data_criacao date NOT NULL, data_envio date, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.expedicao_pedidos (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, expedicao_id uuid NOT NULL REFERENCES public.expedicoes(id) ON DELETE CASCADE, pedido_venda_id uuid NOT NULL REFERENCES public.pedidos_venda(id), created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.pessoas_contato (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, cliente_id uuid NOT NULL REFERENCES public.clientes(id) ON DELETE CASCADE, nome text NOT NULL, setor text, email text, telefone text, ramal text, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.produto_imagens (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, produto_id uuid NOT NULL REFERENCES public.produtos(id) ON DELETE CASCADE, path text NOT NULL, nome_arquivo text NOT NULL, tamanho integer NOT NULL, tipo text NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
CREATE TABLE IF NOT EXISTS public.produto_anuncios (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, produto_id uuid NOT NULL REFERENCES public.produtos(id) ON DELETE CASCADE, ecommerce text NOT NULL, identificador text NOT NULL, descricao text, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
...
WITH receitas AS (
    SELECT date_trunc('month', f.data_emissao) as mes, sum(cr.valor) as total_receita
...
despesas AS (
    SELECT date_trunc('month', cp.data_pagamento) as mes, sum(cp.valor) as total_despesa
    FROM contas_pagar cp
    WHERE cp.status = 'PAGO' AND cp.data_pagamento IS NOT NULL
    GROUP BY 1
)
SELECT
    EXTRACT(YEAR FROM COALESCE(r.mes, d.mes)) as ano,
    EXTRACT(MONTH FROM COALESCE(r.mes, d.mes)) as mes,
    to_char(COALESCE(r.mes, d.mes), 'TMMonth') as mes_nome,
...
DROP POLICY IF EXISTS "Public read access" ON public.servicos;
...
DROP POLICY IF EXISTS "Public read access" ON public.embalagens;
...
DROP POLICY IF EXISTS "Public read access" ON public.pedidos_venda;
...
DROP POLICY IF EXISTS "Public read access" ON public.pedido_venda_itens;
...
DROP POLICY IF EXISTS "Public read access" ON public.pedido_venda_anexos;
...
DROP POLICY IF EXISTS "Public read access" ON public.faturas_venda;
...
DROP POLICY IF EXISTS "Public read access" ON public.fatura_venda_itens;
...
DROP POLICY IF EXISTS "Public read access" ON public.contas_receber;
...
DROP POLICY IF EXISTS "Public read access" ON public.contas_receber_anexos;
...
DROP POLICY IF EXISTS "Public read access" ON public.contas_pagar;
...
DROP POLICY IF EXISTS "Public read access" ON public.contas_pagar_anexos;
...
DROP POLICY IF EXISTS "Public read access" ON public.fluxo_caixa;
...
DROP POLICY IF EXISTS "Public read access" ON public.ordens_servico;
...
DROP POLICY IF EXISTS "Public read access" ON public.ordem_servico_itens;
...
DROP POLICY IF EXISTS "Public read access" ON public.ordem_servico_anexos;
...
DROP POLICY IF EXISTS "Public read access" ON public.ordens_compra;
...
DROP POLICY IF EXISTS "Public read access" ON public.ordem_compra_itens;
...
DROP POLICY IF EXISTS "Public read access" ON public.ordem_compra_anexos;
...
DROP POLICY IF EXISTS "Public read access" ON public.expedicoes;
...
DROP POLICY IF EXISTS "Public read access" ON public.expedicao_pedidos;
...
DROP POLICY IF EXISTS "Public read access" ON public.pessoas_contato;
...
DROP POLICY IF EXISTS "Public read access" ON public.produto_imagens;
...
DROP POLICY IF EXISTS "Public read access" ON public.produto_anuncios;
...
DROP POLICY IF EXISTS "Public read access" ON public.produtos_fornecedores;
...
DROP POLICY IF EXISTS "Public read access" ON public.cliente_anexos;
