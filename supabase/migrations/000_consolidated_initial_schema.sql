...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'frete_por_conta') THEN
        CREATE TYPE public.frete_por_conta AS ENUM ('Contratação do Frete por conta do Remetente (CIF)', 'Contratação do Frete por conta do Destinatário (FOB)', 'Contratação do Frete por conta de Terceiros', 'Transporte Próprio por conta do Remetente', 'Transporte Próprio por conta do Destinatário', 'Sem Ocorrência de Transporte');
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.clientes (
    ...
    contribuinte_icms public.contribuinte_icms,
...
CREATE TABLE IF NOT EXISTS public.vendedores (
    ...
    cpf_cnpj text,
    contribuinte public.contribuinte_icms NOT NULL,
    inscricao_estadual text,
    cep text,
    cidade text,
    uf text,
    ...
    regra_liberacao_comissao public.regra_comissao NOT NULL,
    tipo_comissao public.tipo_comissao_vendedor NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.produtos (
    ...
    codigo_barras text,
    codigo text,
    origem public.origem_produto NOT NULL,
    ...
    embalagem public.embalagem_produto,
    largura numeric,
    altura numeric,
    comprimento numeric,
...
CREATE TABLE IF NOT EXISTS public.pedidos_venda (
    ...
    marcadores text[],
    status public.status_pedido_venda NOT NULL,
    peso_bruto numeric,
    peso_liquido numeric,
...
CREATE TABLE IF NOT EXISTS public.faturas_venda (
    ...
    valor_total numeric NOT NULL,
    status public.status_fatura NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.contas_receber (
    ...
    data_vencimento date NOT NULL,
    data_pagamento date,
    status public.status_conta_receber NOT NULL,
    ocorrencia public.ocorrencia_conta,
    forma_recebimento text,
...
CREATE TABLE IF NOT EXISTS public.contas_pagar (
    ...
    data_vencimento date NOT NULL,
    data_pagamento date,
    status public.status_conta_pagar NOT NULL,
    fornecedor_id uuid REFERENCES public.clientes(id),
    forma_pagamento text,
    numero_documento text,
    historico text,
    categoria_id text,
    ocorrencia public.ocorrencia_conta NOT NULL,
    competencia text,
    marcadores text[],
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL
...
CREATE TABLE IF NOT EXISTS public.ordens_compra (
    ...
    marcadores text[],
    observacoes_internas text,
    status public.status_pedido_venda,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.expedicoes (
    ...
    status public.status_expedicao NOT NULL,
...
CREATE OR REPLACE VIEW public.dre_mensal AS
 SELECT
    ...
    to_char(f.data_emissao, 'TMMonth') AS mes_nome,
    sum(CASE WHEN cr.status = 'RECEBIDO' THEN cr.valor ELSE 0 END) AS receita,
    (SELECT sum(cp.valor) FROM contas_pagar cp WHERE cp.status = 'PAGO' AND EXTRACT(YEAR FROM cp.data_pagamento) = EXTRACT(YEAR FROM f.data_emissao) AND EXTRACT(MONTH FROM cp.data_pagamento) = EXTRACT(MONTH FROM f.data_emissao)) AS despesa,
    (sum(CASE WHEN cr.status = 'RECEBIDO' THEN cr.valor ELSE 0 END) - (SELECT sum(cp.valor) FROM contas_pagar cp WHERE cp.status = 'PAGO' AND EXTRACT(YEAR FROM cp.data_pagamento) = EXTRACT(YEAR FROM f.data_emissao) AND EXTRACT(MONTH FROM cp.data_pagamento) = EXTRACT(MONTH FROM f.data_emissao))) AS resultado
